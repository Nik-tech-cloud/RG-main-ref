...
export class GLTFLoader {
    ...
    async loadLight(nameOrIndex){
        ...
        let light = new Light(gltfSpec);
        this.cache.set(gltfSpec, light);
        return light;
    }

    async loadNodes(...namesOrIndexes) {
        const arr = []
        for(const nameOrIndex of namesOrIndexes){
            arr.push(await this.loadNode(nameOrIndex))
        }
        return arr
    }
    ...
    async loadNode(nameOrIndex) {
        ...
        if (gltfSpec.extensions !== undefined) {
            options.light = await this.loadLight(gltfSpec.extensions.KHR_lights_punctual.light)
        }
